...
const Notificatio = ({ tableId, tableName, setStatus = () => {} }) => {
  ...
  return (
    <>
      <AdminIconButton aria-label="Xem thông báo" onClick={() => setOpen(true)}>
        <BellOutlined />
      </AdminIconButton>
      <Modal
        ...
        <NotificationList>
          {notifications.length ? (
            notifications.map((item, index) => (
              <NotificationItem key={item.notificationId || index}>
                <div className="notice-icon">
                  <CustomerServiceOutlined />
                </div>
                <div>
                  <strong>{item?.text || "Khách cần hỗ trợ"}</strong>
                  <span>Yêu cầu số {index + 1}</span>
                </div>
                <span className="time">
                  {convertToTime(item?.notificationTime) || "Vừa xong"}
                </span>
              </NotificationItem>
            ))
          ) : (
            ...
            >
              <InboxOutlined
                style={{ display: "block", fontSize: 34, marginBottom: 10 }}
              />
